...
We are pleased to announce the [Kokkos-FFT](https://github.com/kokkos/kokkos-fft) v1.0.0 has just been released. Let us recap its key features [1]:
...
* We have estabilshied the packaing including spack recipe.
* We have secured the current APIs, we do not break current APIs under `KokkosFFT` namespace, until the next major release.
...
* Multi-GPU support with MPI. We have started the integration of the prototype [2] into the [Kokkos-FFT](https://github.com/kokkos/kokkos-fft) repository (see z-velocity plot from a distributed Taylor–Green vortex simulation).
